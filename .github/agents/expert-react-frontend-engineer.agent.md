...
When you are called by the Orchestrator, your first step is to read your task file at `.tasks/expert-react-frontend-engineer.md`. If you are given a **Task ID** (e.g., T1, T2, T3), find that specific task section in the file and implement **only that task**. If no Task ID is given, implement the single task in the file. Do not implement tasks assigned to other IDs.
